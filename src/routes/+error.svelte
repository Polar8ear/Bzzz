...
<div class="mx-4 flex min-h-screen flex-col items-center justify-center">
	<div>
		<p class="opacity-50">Error Occured:</p>
		<h1 class="text-3xl">{$page.error?.message}</h1>
		{#if $page.error?.errorId != null}
			<p>
				You can contact the system admin with this error id ({$page.error?.errorId}) to let them
				know the investigate
			</p>
		{/if}
